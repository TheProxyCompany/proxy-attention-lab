...
//  Fetches a K‑vector for a (token,q‑head) pair and computes the scaled dot product.
...
[[kernel]] void paged_attn_kernel(
    ...
    // --- Get pointer to K-vector ---
    device const half* k_vector_ptr = k_cache_pool_in + k_vector_start_idx;

    // --- Compute Dot Product Q·K^T ---
    float score_sum_float = 0.0f; // Accumulate in float for precision
    for (uint i = 0; i < params->head_dim; ++i) {
        score_sum_float += (float)q_vector_ptr[i] * (float)k_vector_ptr[i];
    }

    // Apply scale
    score_sum_float *= params->scale;
    half score_half = (half)score_sum_float;

    // Write scalar dot product score to output
    output_buffer[global_query_thread_idx] = score_half;
}
